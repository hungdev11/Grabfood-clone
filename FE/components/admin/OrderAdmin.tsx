...
export default function OrderList() {
  ...
  const handleCancelOrder = async (orderId: number) => {
    try {
        await axios.put(`http://localhost:6969/grab/order/admin/cancel/${orderId}`);
      ...
      <div className="space-y-6">
        {orders.map((order) => (
          ...
          >
            ...
            <p className="text-sm">Tổng giá: {(order.totalPrice + order.shippingFee).toLocaleString()}đ</p>
            <div className="mt-3">
                ...
                <div key={item.id} className="flex items-center gap-4 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Thêm: {item.additionFoods.map((add) => add.name).join(', ')}
                    </p>
                  </div>
                </div>
              ))}
